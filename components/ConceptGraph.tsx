...
import { Card, Spinner, Text } from "@rewind-ui/core";
...
const ConceptGraph: React.FC<ConceptGraphProps> = ({ rootConcept, onNodeClick, currentConceptId }) => {
  ...
  return (
    <Card className="flex-grow flex flex-col">
      <Card.Header>
        <Text variant="h2" weight="bold" className="text-gray-900">Related Concepts Graph</Text>
        <Text size="xs" color="gray" className="text-gray-500">Click node to explore. Drag background to pan. Zoom with scroll.</Text>
      </Card.Header>
      <Card.Body className="flex-grow flex flex-col p-0"> {/* Adjusted padding for Card.Body if needed */}
        {isLoadingGraph && (
        ...
        <div className={`${styles.graphContainer} ${isLoadingGraph ? "hidden" : ""} flex-grow`}>
          <svg ref={svgRef} className="w-full h-full"></svg> {/* Ensure SVG takes up space */}
        </div>
      </Card.Body>
    </Card>
  );
